...
namespace GLaDOS {
  ...
  bool Texture2D::loadTextureFromFile() {
    ...
    LOG_TRACE("Texture load success: [name={0}, width={1}, height={2}, bpp={3}, useMipmap={4}, mipmapCount={5}]", mName, mWidth, mHeight, mChannels, mUseMipmap, mMipmapCount);
  ...
  bool Texture2D::generateMipmapsTexture(uint32_t x, uint32_t y, uint8_t* data) {
    ...
    for (uint32_t level = 1; level <= mMipmapCount; level++) {
      ...
      if (stbir_resize_uint8(data, mWidth, mHeight, 0, mipData, mipWidth, mipHeight, 0, mChannels) == 0) {
        LOG_ERROR("Failed to create mipmap [name={0}, width={1}, height={2}, level={3}, bpp={4}]", mName, mipWidth, mipHeight, level, mChannels);
        return false;
      }
      replaceRegion(x, y, mipWidth, mipHeight, level, mipData);
      LOG_TRACE("Mipmap successfully created [name={0}, width={1}, height={2}, level={3}, bpp={4}]", mName, mipWidth, mipHeight, level, mChannels);
      FREE(mipData);
    }
