...
namespace GLaDOS {
  ...
  constexpr MTLCompareFunction MetalDepthStencilState::mapComparisonFunctionFrom(ComparisonFunction func) {
    switch (func) {
      ...
      default:
        LOG_WARN("Unknown Depth comparison function! fallback to ComparisonFunction::Less");
        break;
    }
  ...
  constexpr MTLStencilOperation MetalDepthStencilState::mapStencilOperatorFrom(StencilOperator op) {
    switch (op) {
      ...
      default:
        LOG_WARN("Unknown Stencil operator! fallback to StencilOperator::Keep");
        break;
    }
  ...
  constexpr MTLSamplerMinMagFilter MetalSamplerState::mapSamplerMinMagFilterFrom(FilterMode mode) {
    switch (mode) {
      ...
      default:
        LOG_WARN("Not recognized FilterMode in metal renderer fallback to Nearest");
    }
  ...
  constexpr MTLSamplerAddressMode MetalSamplerState::mapSamplerAddressModeFrom(WrapMode mode) {
    switch (mode) {
      ...
      default:
        LOG_WARN("Not recognized WrapMode in metal renderer fallback to Clamp");
        break;
    }
