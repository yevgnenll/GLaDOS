...
namespace GLaDOS {
  ...
  bool TextureCube::loadTextureFromFile(Vector<std::string>& names) {
    ...
    for (std::string const& name : names) {
      ...
      if (result == 0) {
        LOG_ERROR("Failed to get info texture {0}, reason: {1}", filename, stbi_failure_reason());
        return false;
      }
      if (width != height) {
        LOG_ERROR("Failed to load texture {0}: Cubemap texture must be uniformly-sized.", filename);
        return false;
      }
      ...
      if (data == nullptr) {
        LOG_ERROR("Failed to load texture {0}, reason: {1}", filename, stbi_failure_reason());
        return false;
      }
    ...
    if (!isSame) {
      LOG_ERROR("All cubemap texture must be uniformly-sized and same channel bits.");
    ...
    for (auto& image : images) {
      LOG_TRACE("CubeTexture load success: [name={0}, width={1}, height={2}, bpp={3}]", image.name, image.width, image.height, image.channels);
      stbi_image_free(image.data);
    }
