...
namespace GLaDOS {
  ...
  OSVersion CocoaPlatform::makeOSVersion(uint32_t major, uint32_t minor, uint32_t patch) {
    return (OSVersion)major + ((OSVersion)minor / 100.f) + ((OSVersion)patch / 10000.f);
  }
  ...
  bool CocoaPlatform::OSVersionIsAtLeast(OSVersion minor) {
    return mCurrentOSVersion >= minor;
  }
